...
lane_config = load_json(os.path.join(PROJECT_ROOT, "data", "lane_coordinates.json"))

INPUT_DIR = os.path.join(TRAINING_DIR, "data", "1_raw_frames")
OUTPUT_DIR = os.path.join(TRAINING_DIR, "data", "2_processed_dataset")
...
for img_file in [f for f in os.listdir(INPUT_DIR) if f.endswith(".jpg")]:
    ...
    for lane in lane_config["lanes"]:
        ...
        if lane_labels:
            ...
            cv2.imwrite(os.path.join(OUTPUT_DIR, f"{out_name}.jpg"), warped_lane)
            with open(os.path.join(OUTPUT_DIR, f"{out_name}.txt"), "w") as f_out:
                f_out.write("\n".join(lane_labels))
